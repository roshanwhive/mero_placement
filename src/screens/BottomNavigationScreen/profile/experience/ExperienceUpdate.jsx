...
import {updateExperience} from '../../../../features/profile/testSlice/ExperienceSlice';
...
const ExperienceUpdate = id => {
  ...
  const {singleExperience, isLoading} = useSelector(
    state => state.getSingleExperience,
  );
  ...
  useEffect(() => {
    if (singleExperience) {
      reset({
        ...
        start_date:
          new Date(singleExperience?.experience?.start_date) || new Date(),

        end_date: singleExperience?.experience?.end_date
          ? new Date(singleExperience?.experience?.end_date)
          : new Date(),
  ...
  const onPressUpdate = handleSubmit(async experienceData => {
    dispatch(updateExperience(experienceData)).then(() => {
      navigation.navigate('ExperienceList');
      console.log('eduDataAdd', experienceData);
    });
  });
